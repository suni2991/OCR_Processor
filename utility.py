...
def create_zip_file(excel_dir):
    ...
    zip_filename = os.path.join('./download', zip_filename)
    
    with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for root, dirs, files in os.walk(excel_dir):
            for file in files:
                file_path = os.path.join(root, file)
                zipf.write(file_path, os.path.relpath(file_path, excel_dir))
    
    # Delete the Excel files after compressing them
    for root, dirs, files in os.walk(excel_dir):
        for file in files:
            file_path = os.path.join(root, file)
            os.remove(file_path)
...
def split_pdf(input_pdf_path : str, pages_per_split : int = 10):
    ...
    unique_filename = input_pdf_path.split('\\')[-1].split('.pdf')[0]
    ...
    for split_index in range(num_splits):
        ...
        output_pdf_path = os.path.join(output_dir, f'{unique_filename}_split_{split_index + 1}.pdf')
...
def combine_excels(excel_files : list, unique_filename : str):
    ...
    output_excel_file = os.path.join( output_DIR,  f'{unique_filename}_combined.xlsx' )
