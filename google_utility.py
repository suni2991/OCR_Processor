...
# Function to translate a document using Google Cloud Translation API v3
...
def process_pdf( document : documentai.Document, unique_filename : str ):
    '''
    ...
    unique_filename              name of the pdf file (without the extension)
    ...
    for page in document.pages:
        ...
        for table in page.tables:
            ...
            if ctr == 1:
                ...
                df.to_excel(os.path.join(excel_output_DIR, f'{unique_filename}.xlsx'), sheet_name = str(ctr))
                print(f'Processed page: {ctr}')
                print('-------------------------------------------------')
                end_time = time.time()
                
            else:
                with pd.ExcelWriter(os.path.join(excel_output_DIR, f'{unique_filename}.xlsx'), engine='openpyxl', mode='a', if_sheet_exists='new') as writer:
